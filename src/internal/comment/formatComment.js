...
import { renderFileSizeImpactTable } from "./renderFileSizeImpact.js"
...
const renderCommentBody = ({
  ...
  const groupMessages = Object.keys(snapshotComparison).map((groupName) => {
    ...
    const addImpact = (fileRelativeUrl, { event, sizeImpactMap, base, afterMerge }) => {
      ...
      if (hasSizeImpact(sizeImpactMap)) {
        if (metaEnables(meta, "showFileSizeImpact", hookParameters)) {
  ...
  <p>No impact on files in ${groupName} group.</p>
    ...
    const elements = [
      ...
      ...(sizeImpactCount
        ? [renderFileSizeImpactTable(sizeImpacts, { transformations, formatSize })]
        : []),
      ...(cacheImpactCount
        ? [
            renderCacheImpactDescription(cacheImpacts),
            renderCacheImpactTable(cacheImpacts, { transformations, formatSize }),
          ]
        : []),
      ...(sizeImpactExcludedCount > 0 || cacheImpactExcludedCount > 0
        ? [
            formulateExcludedImpacts({
              sizeImpactExcluded,
...
const formulateExcludedImpacts = ({
  sizeImpactExcluded,
  cacheImpactsExcluded,
  transformations,
  formatSize,
  ...
  return `<details>
  <summary>Show excluded impacts (${impactExcludedCount})</summary>
  <p>${formulateExcludedImpact({ sizeImpactExcludedCount, cacheImpactExcludedCount })}.</p>
  ${elements.join(`
  `)}
  </details>`
}

const formulateExcludedImpact = ({ sizeImpactExcludedCount, cacheImpactExcludedCount }) => {
